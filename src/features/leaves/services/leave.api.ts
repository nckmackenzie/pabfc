...
async function initialiseLeaveBalancesForEmployeeRows({
	employeeId,
	leaveYear,
}: {
	employeeId: string;
...
export const initialiseLeaveBalancesForEmployee = createServerFn({ method: "POST" })
	...
	.handler(async ({ data: { employeeId, leaveYear } }) => {
		await requireAnyPermission(["employees:create", "leaves:adjust-balances"]);
			...
			try {
				const approvedRequest = await db.transaction(async (tx) => {
					...
					if (!entitlement.isOneOffEntitlement) {
						...
						if (!balanceUpdate) {
							throw new Error("Leave balance row not found while applying request");
						}
					}
