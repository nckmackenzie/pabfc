...
import { accessControlSyncJobs, biotimePersonProfiles, employees } from "@/drizzle/schema";
import {
...
const employeeUniqueFieldDefinitions = [
  ...
  key: keyof Pick<
    EmployeeSchema,
    "phone" | "email" | "nationalId" | "kraPin" | "nssfNo" | "shifNo" | "helbRef"
  >;
  column:
...
async function employeeNoExists(employeeNo: string, excludeEmployeeId?: string) {
...
async function createEmployee({
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      const isNotActive = data.status === "resigned" || data.status === "terminated";
      ...
      if (!leaveBalanceResult.success) {
        console.error(
          "Failed to initialize leave balances for employee",
          employeeResult.employeeId,
          leaveBalanceResult.error
        );
      }
...
async function updateEmployee({
  ...
  const conflictMessage = await getEmployeeConflictMessage(employeeValues, data.id);
  if (conflictMessage) {
    return failure({
      type: "ConflictError",
      message: conflictMessage,
  ...
  try {
    await db.transaction(async (tx) => {
      await tx.update(employees).set(employeeValues).where(eq(employees.id, existingEmployee.id));
      ...
      if (employeeProfile) {
        const wasTerminal =
          existingEmployee.status === "resigned" || existingEmployee.status === "terminated";
        const isTerminal = data.status === "resigned" || data.status === "terminated";
        ...
        if (!wasTerminal && isTerminal) {
          ...
          await tx.insert(accessControlSyncJobs).values({
            ...
            payload: {
              ...
              reason: data.status === "resigned" ? "membership_expired" : "employment_terminated",
            },
        ...
        if (
          data.status === "active" &&
          (existingEmployee.status === "terminated" || existingEmployee.status === "resigned")
        ) {
          await tx
            .update(biotimePersonProfiles)
            .set({
