...
import { biometricEnrollmentStatusEnum, members } from "@/drizzle/schemas/member";
import { employees } from "./employees";
import { accessControlStatusEnum, accessSyncActionEnum, accessSyncStatusEnum } from "./member";

export const biotimePersonTypeEnum = pgEnum("biotime_person_type", ["member", "employee"]);
...
export const accessControlSyncJobs = pgTable(
	"access_control_sync_jobs",
	{
		...
		personType: biotimePersonTypeEnum("person_type").notNull().default("member"),
...
export const biotimeSettings = pgTable("biotime_settings", {
	id: uuid("id").primaryKey().defaultRandom(),
	baseUrl: varchar("base_url", { length: 255 }).notNull().default("http://127.0.0.1/"),
...
export const biotimePersonProfiles = pgTable(
	"biotime_person_profiles",
	{
		...
		desiredAccessEnabled: boolean("desired_access_enabled").notNull().default(false),
		accessControlStatus: accessControlStatusEnum("access_control_status")
			.notNull()
			.default("not_synced"),
		biometricEnrollmentStatus: biometricEnrollmentStatusEnum("biometric_enrollment_status")
			.notNull()
			.default("pending"),
	...
	(table) => [
		uniqueIndex("uq_biotime_person_profiles_emp_code").on(table.biotimeEmployeeCode),
		uniqueIndex("uq_biotime_person_profiles_biotime_id").on(table.biotimeEmployeeId),
...
export const employeeGymAccessLogs = pgTable("employee_gym_access_logs", {
